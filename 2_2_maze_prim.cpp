...
void maze_prim :: generate_maze(maze& Maze, int row, int col)
{
    ...
    Maze.way[row][col] = 1;
    Maze.draw_cell(Maze.renderer, row, col, red, Maze.cell_size);
    SDL_RenderPresent(Maze.renderer);

    vector<int> drow = {0, 0, 2, -2}, dcol = {-2, 2, 0, 0};
    for (int i = 0; i < 4 ;i++){
    ...
    while (!edge.empty()){
        ...
        if (! Maze.visited[end_row][end_col]){
            ...
            Maze.draw_cell(Maze.renderer, start_row, start_col, black,  Maze.cell_size);
